...
const getSeverityColor = (severity: NoticeSeverity): NoticeColor => {
  switch (severity) {
    case 'info':
      return 'primary';
    case 'success':
...
const getSeverityIcon = (severity: NoticeSeverity): React.ReactNode => {
  switch (severity) {
    case 'info':
...
function NoticeBase(props: NoticeProps, ref: React.Ref<View>) {
  const {
    variant = 'light',
    color = 'primary',
    sev,
  ...
  const alertStyles = getNoticeStyles();
...
export const Notice = factory<NoticeFactoryPayload>(NoticeBase);
