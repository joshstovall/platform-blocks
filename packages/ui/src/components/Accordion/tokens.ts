...
export const ACCORDION_VARIANTS: Record<string, AccordionVariantTokenFn> = {
  // Use semantic surface tokens (theme.backgrounds.*) so the Accordion blends
  // into its surrounding surface and adapts across light/dark themes.
  default: (theme) => ({
    container: { backgroundColor: 'transparent' },
    item: { backgroundColor: 'transparent', borderBottomWidth: 1, borderBottomColor: theme.backgrounds.border },
    header: { backgroundColor: 'transparent' },
    content: { backgroundColor: 'transparent' },
  }),
  separated: (theme, radius) => ({
    container: { backgroundColor: 'transparent' },
    item: { backgroundColor: theme.backgrounds.subtle, borderWidth: 1, borderColor: theme.backgrounds.border, marginBottom: 8, ...(radius || { borderRadius: 8 }) },
