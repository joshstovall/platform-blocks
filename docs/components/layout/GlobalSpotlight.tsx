  ...
  return (
    <Spotlight.Root query={state.query} onQueryChange={setQuery} opened={state.opened} onClose={() => { setQuery(''); close(); }}>
      ...
      <Spotlight.ActionsList>
        {spotlightActions.length > 0 ? (() => { let flatIdx = 0; return spotlightActions.map((item, groupIdx) => { if ('group' in item) { return (
          <Spotlight.ActionsGroup key={`group-${groupIdx}`} label={item.group}>
            {item.actions.map(action => { const isSelected = flatIdx === selectedIndex; flatIdx++; return (
              <Spotlight.Action key={action.id} label={action.label} description={action.description} selected={isSelected} leftSection={action.icon ? (<Icon name={action.icon as any} size="md" />) : undefined} onPress={() => { 
        ...
        ); } else { const isSelected = flatIdx === selectedIndex; flatIdx++; return (
          <Spotlight.Action key={item.id} label={item.label} description={item.description} selected={isSelected} leftSection={item.icon ? (<Icon name={item.icon as any} size="md" />) : undefined} onPress={() => { 
            console.log('=== SPOTLIGHT ACTION ONPRESS CALLED ==='); 
